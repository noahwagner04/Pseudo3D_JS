...
// havent implemented the tint feature yet.
Pseudo3D.Camera.prototype.renderSprites = function(scene, renderer) {
	...
	for (var i = sprites.length - 1; i >= 0; i--) {
		...
		if (transformY > this.farClippingPlane) continue;
		...
		var drawStartY = ~~(remapLocal((-worldHeight - sprite.position.z) / this.planeLength / transformY, -1, 1, 0, height));
		var drawEndY = ~~(remapLocal((worldHeight - sprite.position.z) / this.planeLength / transformY, -1, 1, 0, height));
		...
		var drawStartX = ~~(remapLocal((transformX - worldWidth / this.planeLength) / transformY, -aspectRatio, aspectRatio, 0, width));
		var drawEndX = ~~(remapLocal((transformX + worldWidth / this.planeLength) / transformY, -aspectRatio, aspectRatio, 0, width));
...
Pseudo3D.Camera.prototype.renderScene = function(scene, renderer) {
	...
	renderer.depthBuffer = Array(renderer.renderWidth).fill().map(() => Array(renderer.renderHeight).fill(Infinity));
	if (scene.skybox) {
	...
	if (scene.floor.provided || scene.ceiling.provided) {
	...
	var imageData = new ImageData(renderer.pixels, renderer.renderWidth)
