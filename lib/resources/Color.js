...
Pseudo3D.Color = function(colorArray, type) {
	...
	if (type === Pseudo3D.ColorTypes.HSB) {
		this.hsbArray = new Uint8ClampedArray(4);
		this.hsbArray[0] = colorArray[0];
		this.hsbArray[1] = colorArray[1];
		this.hsbArray[2] = colorArray[2];
		this.hsbArray[3] = Pseudo3D.Math.constrain(colorArray[3], 0, 100);
		var rgb = Pseudo3D.Color.HSBToRGB(
			this.hsbArray[0],
			this.hsbArray[1],
			this.hsbArray[2],
			this.hsbArray[3]
...
Pseudo3D.Color.HSBToRGB = function(h, s, b, a) {
