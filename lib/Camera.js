...
Pseudo3D.Camera.prototype.renderFloorCeiling = function(scene, renderer) {
	...
	for (var y = 0; y < height; y++) {
		...
		var floorStepX = (rayDirX1 - rayDirX0) * scalar;
		var floorStepY = (rayDirY1 - rayDirY0) * scalar;
...
Pseudo3D.Camera.prototype.renderSprites = function(scene, renderer) {
	...
	for (var i = sprites.length - 1; i >= 0; i--) {
		...
		drawStartY = ~~(remapLocal((-worldHeight - sprite.position.z) / this.planeLength / transformY, -1, 1, 0, height));
		drawStartX = ~~(remapLocal((-worldHeight - sprite.position.z) / this.planeLength / transformY, -1, 1, 0, height));
		*/
		...
		*/
		...
		var drawStartX = ~~(((transformX - worldWidth) / denominator + aspectRatio) * scalar);
		var drawEndX = ~~(((transformX + worldWidth) / denominator + aspectRatio) * scalar);
