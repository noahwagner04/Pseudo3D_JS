...
Pseudo3D.Renderer = function(width, height, quality) {
	...
	if(!this.canvas.getContext || !this.canvas.getContext('2d')) {
		console.log("your browser does not support the canvas element, cannot render raycasting scene")
		return;
	}
...
Pseudo3D.Renderer.prototype.render = function(scene, camera) {
	...
	if(!camera.type) {
		console.log("cannot render scene with camera that is not of type ray or voxel");
		return;
	}
	this.depthBuffer = Array(this.renderWidth).fill().map(() => Array(this.renderHeight).fill(Infinity));
