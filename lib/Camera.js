...
Pseudo3D.Camera = function(config) {
	...
	if (!Pseudo3D.Entity) {
		console.log("cannot create Pseudo3D.Camera because Pseudo3D.Entity class does not exist");
		return;
	}
...
// when the document if fully loaded, only then try to inheret from entity class
window.addEventListener('load', function() {
	if (!Pseudo3D.Entity) {
		console.log("Pseudo3D.Entity does not exist, cannot create Pseudo3D.Camera objects properly")
	}
