	...
	*/
	function Ray(posX, posY, dirX, dirY) {
		...
		if (posX instanceof Pseudo3D.Camera) {
			...
			this.direction.add(camera.plane).scale(camera.rayAngle);
		...
		this.mapPosition.x = Math.floor(this.mapPosition.x);
		this.mapPosition.y = Math.floor(this.mapPosition.y);
	...
	function renderCast(scene) {
		...
		while (this.hit === 0) {
			this.increment(scene);
		}
		if (this.side === 0) {
			this.perpendicular = (this.sideDistX - this.deltaDistX) * this.camera.focalLength;
		} else {
			this.perpendicular = (this.sideDistY - this.deltaDistY) * this.camera.focalLength;
		}
	}
	...
	// executes a single step of dda
	Ray.prototype.increment = function(scene) {
