...
Pseudo3D.Scene = (function() {
	...
	function Scene(config) {
		...
		if (this.floor.texture instanceof Pseudo3D.Texture) {
			...
			if (this.floor.cellSize instanceof Array) {
				if (typeof this.floor.cellSize[0] !== "number") {
					this.floor.cellSize[0] = 1;
				}
				if (typeof this.floor.cellSize[1] !== "number") {
					this.floor.cellSize[1] = 1;
				}
			} else {
				this.floor.cellSize = [1, 1];
			}
		...
		if (this.ceiling.texture instanceof Pseudo3D.Texture) {
			...
			if (this.ceiling.cellSize instanceof Array) {
				if (typeof this.ceiling.cellSize[0] !== "number") {
					this.ceiling.cellSize[0] = 1;
				}
				if (typeof this.ceiling.cellSize[1] !== "number") {
					this.ceiling.cellSize[1] = 1;
				}
			} else {
				this.ceiling.cellSize = [1, 1];
			}
