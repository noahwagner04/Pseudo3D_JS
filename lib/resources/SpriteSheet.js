	...
	this.source.html.addEventListener("load", function() {
		...
		for (var i = 0; i < this.source.width; i += this.cellWidth) {
			for (var j = 0; j < this.source.height; j += this.cellHeight) {
				// push a new Pseudo3D.Texture object, pass getPixelData to it and width and height ect.
				this.textures.push(new Pseudo3D.Texture(Pseudo3D.Texture.getPixelData(
					this.source.html,
					i,
					j,
					this.cellWidth,
